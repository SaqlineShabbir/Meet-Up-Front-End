...
import { ChatAlt2Icon, MoonIcon, QuestionMarkCircleIcon } from '@heroicons/react/solid';
  ...
  return (
    <div>
      ...
      <img  className="rounded-full cursor-pointer h-12 w-12" src={user?.photoURL} alt="image" />
        ...
        <ChevronDownIcon onClick={() => setOpenMenu(!openMenu)} className={`icon ${openMenu?"bg-blue-500 text-white":''}`} />
        
      </div>
      
    </div>
    {/* menu  */}
    {openMenu && <div className='absolute top-20 p-2 z-50 rounded-lg shadow right-8 w-80 bg-white'>
      <div className='flex p-2 rounded-lg items-center hover:bg-gray-200 gap-2 mb-4'>
        <img src={user?.photoURL} className="rounded-full w-12 h-12" alt="" />
        <div>
